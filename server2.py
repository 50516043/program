...
def size_request_server(word_list,s):
...
def get_request_server(word_list,s,token_str):    
    filename = word_list[1]
    if word_list[2] == pbl2017.genkey(token_str):
      if word_list[3] == 'ALL':
        try:
          f = open(filename)
          file_size = os.path.getsize(filename)
...
def rep_request_server(word_list,s,token_str):
  try:
